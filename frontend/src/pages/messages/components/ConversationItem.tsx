  ...
  return (
    <button
      ...
      className={`
        ...
        ${isActive
          ? 'bg-linear-to-r from-orange-500/15 to-transparent border-2 border-orange-400/40 rounded-2xl shadow-lg shadow-orange-500/10'
          : 'bg-white/5 hover:bg-white/10 border-2 border-transparent hover:border-orange-500/10 rounded-2xl'
        }
      `}
      style={{ width: 'calc(100% - 16px)' }}
    >
      {/* Hover gradient effect */}
      <div className="absolute inset-0 bg-linear-to-r from-cyan-500/0 via-orange-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      
      {/* Active indicator line */}
      {isActive && (
        <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 rounded-full bg-linear-to-b from-orange-400 to-amber-400" />
      )}
      ...
      <div className="relative shrink-0 z-10">
        {isGroup && participants.length > 1 ? (
          ...
          <div className="relative w-12 h-12">
            <div className="absolute top-0 left-0 w-9 h-9 rounded-xl overflow-hidden border-2 border-[#0e0805] shadow-lg">
              <img src={participants[0]?.avatarUrl} alt="" className="w-full h-full object-cover" />
            </div>
            <div className="absolute bottom-0 right-0 w-9 h-9 rounded-xl overflow-hidden border-2 border-[#0e0805] shadow-lg">
          ...
          <div className="relative">
            ...
            {isActive && (
              <div className="absolute -inset-1 rounded-full border-2 border-orange-400/30 animate-pulse" />
            )}
          </div>
        )}
        ...
        {isOnline && (
          <div className="absolute -bottom-0.5 -right-0.5">
            <div className="w-4 h-4 bg-emerald-500 rounded-full border-2 border-[#0e0805] flex items-center justify-center">
      ...
      <div className="flex-1 min-w-0 z-10">
        <div className="flex items-center gap-1.5">
          ...
          {isVerified && <Icon name="verified" size={14} className="text-orange-400" />}
          {isPinned && (
        ...
        {lastMessage && (
          <p className={`text-sm truncate mt-0.5 ${unreadCount > 0 ? 'text-amber-100 font-medium' : 'text-slate-400'}`}>
            {isOwnMessage && <span className="text-orange-400/70">You: </span>}
            {lastMessage.content}
          </p>
        )}
      </div>
      ...
      <div className="shrink-0 flex flex-col items-end gap-1.5 z-10">
        <span className={`text-xs font-medium ${unreadCount > 0 ? 'text-orange-300' : 'text-slate-500'}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          {unreadCount > 0 && (
            <div className="relative">
              <div className="absolute inset-0 bg-linear-to-r from-orange-400 to-amber-400 rounded-full blur-md opacity-50" />
